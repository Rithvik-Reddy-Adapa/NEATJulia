...
function main()
  global neat_rnn_config = NEAT_RNN_config(n_inputs = 5,
                                           n_outputs = 3,
                                           population_size = 100,
                                           ...
                                           initial_mutation_probability = RNN_Mutation_Probability(shift_weight = 7,
                                                                                                   shift_bias = 3,
                                                                                                   add_forward_connection = 1,
                                                                                                   add_node = 0.1,
                                                                                                   add_recurrent_connection = 0,
                                                                                                   disable_recurrent_connection = 0,
                                                                                                   enable_recurrent_connection = 0.5,
                                                                                                   change_activation_function = 0.0,
                                                                                                  ),
                                          )
  global neat_rnn = NEAT_RNN(config = neat_rnn_config)
  for i in neat_rnn.population
    AddRecurrentConnection(i, 6, 3, 9)
    AddRecurrentConnection(i, 7, 4, 10)
    AddRecurrentConnection(i, 8, 5, 11)
  end
  push!(neat_rnn.GIN, GIN(9, RecurrentConnection, 6, 3))
  push!(neat_rnn.GIN, GIN(10, RecurrentConnection, 7, 4))
  push!(neat_rnn.GIN, GIN(11, RecurrentConnection, 8, 5))
  ...
  for j = 1:length(t)
    ...
    push!(input_sequence, [t[j], amp*sind(360*freq*t[j] + phi), 0,0,0])
  end
