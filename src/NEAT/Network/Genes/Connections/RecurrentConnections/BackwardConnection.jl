#=
# Refer src/NEAT/Network/Genes/Connections/RecurrentConnections/BackwardConnectionConfig.jl for BackwardConnectionConfig, CheckConfig
...
export BackwardConnection, Init, Run, ResetIO, ChangeWeight, ShiftWeight, ToggleEnable, EnableGene, DisableGene

@kwdef mutable struct BackwardConnection <: RecurrentConnections
  GIN::Unsigned = 0 # Global Innovation Number
  in_node::Union{OutputNode, HiddenNodes} # points to the input node
  out_node::Union{InputNode, HiddenNodes} # points to the output node
...
function Base.show(io::IO, x::BackwardConnection)
...
function Init(x::BackwardConnection, config::BackwardConnectionConfig)
  CheckConfig(config)
...
function Run(x::BackwardConnection)
  if x.enabled[]
...
function ResetIO(x::BackwardConnection)
  x.input[] = missing
  x.output[] = missing
  return
end

function ChangeWeight(x::BackwardConnection, config::BackwardConnectionConfig)
  CheckConfig(config)
...
function ShiftWeight(x::BackwardConnection, config::BackwardConnectionConfig)
  CheckConfig(config)
...
function ToggleEnable(x::BackwardConnectionConfig)
  x.enabled[] = !(x.enabled[])
  return x.enabled[]
end

function EnableGene(x::BackwardConnectionConfig)
  x.enabled[] = true
  return
end

function DisableGene(x::BackwardConnectionConfig)
  x.enabled[] = false
  return
end
